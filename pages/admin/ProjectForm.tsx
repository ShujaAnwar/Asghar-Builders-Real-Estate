    ...
    <div className="min-h-screen bg-slate-950 pt-32 pb-24 px-4">
      <div className="max-w-5xl mx-auto">
        ...
        <header className="flex flex-col md:flex-row justify-between items-start md:items-end mb-12 gap-8">
          <div>
            <h1 className="text-5xl font-black text-white">{isEditing ? 'Sync Build' : 'Deploy Landmark'}</h1>
        ...
        <form onSubmit={handleSubmit} className="space-y-12">
              ...
              <div className="glass p-10 rounded-[3rem] border border-white/10 space-y-10">
                <div className="grid md:grid-cols-2 gap-8">
                  <div className="space-y-3">
                    ...
                    <input required value={formData.name} onChange={e => handleNameChange(e.target.value)} className="w-full bg-white/5 border border-white/10 rounded-2xl py-5 px-8 text-white focus:border-amber-500 outline-none" />
                  </div>
                  <div className="space-y-3">
                    <label className="text-[10px] font-black text-gray-500 uppercase tracking-widest px-1">Location Node</label>
                    <input required value={formData.location} onChange={e => setFormData({ ...formData, location: e.target.value })} className="w-full bg-white/5 border border-white/10 rounded-2xl py-5 px-8 text-white focus:border-amber-500 outline-none" />
                  </div>
                </div>
                ...
                <div className="grid md:grid-cols-3 gap-8">
                  <div className="space-y-3">
                    ...
                    <select value={formData.type} onChange={e => setFormData({ ...formData, type: e.target.value as ProjectType })} className="w-full bg-slate-900 border border-white/10 rounded-2xl py-5 px-8 text-white outline-none">
                      {Object.values(ProjectType).map(t => <option key={t} value={t}>{t}</option>)}
                    </select>
                  </div>
                  <div className="space-y-3">
                    <label className="text-[10px] font-black text-gray-500 uppercase tracking-widest px-1">Status</label>
                    <select value={formData.status} onChange={e => setFormData({ ...formData, status: e.target.value as ProjectStatus })} className="w-full bg-slate-900 border border-white/10 rounded-2xl py-5 px-8 text-white outline-none">
              ...
              <div className="glass p-10 rounded-[3rem] border border-white/10 space-y-10">
                ...
                {formData.specs?.map((spec, i) => (
                  <div key={i} className="flex gap-4">
                    <input value={spec.label} onChange={e => updateSpec(i, 'label', e.target.value)} placeholder="Label" className="flex-1 bg-white/5 border border-white/10 rounded-xl py-4 px-6 text-white" />
                    <input value={spec.value} onChange={e => updateSpec(i, 'value', e.target.value)} placeholder="Value" className="flex-1 bg-white/5 border border-white/10 rounded-xl py-4 px-6 text-white" />
          ...
          {activeTab === 'media' && (
            <div className="space-y-10 animate-in fade-in slide-in-from-bottom-4">
              <div className="glass p-10 rounded-[3rem] border border-white/10 space-y-8">
                <h3 className="text-xl font-bold text-white">Master Billboard</h3>
                <div className="flex flex-col md:flex-row gap-10">
                  <div className="w-full md:w-80 aspect-video glass rounded-3xl overflow-hidden relative group border border-white/10">
                    {formData.imageUrl ? (
                      <img src={formData.imageUrl} className="w-full h-full object-cover" />
                    ) : (
                      <div className="w-full h-full flex flex-col items-center justify-center text-gray-600 bg-slate-900/50">
                  ...
                  <div className="flex-1 space-y-4">
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                      <button 
                        ...
                        className="flex flex-col items-center justify-center p-8 border-2 border-dashed border-white/10 rounded-3xl hover:border-amber-500/50 hover:bg-amber-500/5 transition-all group"
                      >
                        <Upload className="mb-2 text-amber-500 group-hover:scale-110 transition-transform" />
                        <span className="text-xs font-black uppercase tracking-widest text-white">Browse Local PC</span>
                      </button>
                      <button 
                        type="button"
                      ...
                      >
                        ...
                        <span className="text-xs font-black uppercase tracking-widest text-white">Cloud Library</span>
                      </button>
                    </div>
                    <div className="space-y-2">
                       ...
                       <input 
                        ...
                        className="w-full bg-white/5 border border-white/10 rounded-2xl py-4 px-8 text-white focus:border-amber-500 outline-none text-sm font-mono" 
                        placeholder="https://..."
                      />
                    </div>
                  </div>
              ...
              <div className="glass p-10 rounded-[3rem] border border-white/10 space-y-8">
                <div className="flex justify-between items-center">
                  <h3 className="text-xl font-bold text-white">Image Portfolio (Gallery)</h3>
                  <button 
                    type="button" 
                    onClick={() => setShowPicker('gallery')}
                    className="px-6 py-2 bg-blue-500/10 text-blue-400 border border-blue-500/20 rounded-xl text-xs font-black uppercase tracking-widest hover:bg-blue-500 hover:text-white transition-all"
                  >
                    Select from Assets
                  </button>
                </div>
                <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
                  ...
                  <button 
                    type="button"
                    onClick={() => setShowPicker('gallery')}
                    className="aspect-square glass rounded-2xl border border-dashed border-white/20 flex flex-col items-center justify-center text-gray-600 hover:border-amber-500/50 hover:text-amber-500 transition-all"
                  >
                    <Plus size={32} />
          ...
          {activeTab === 'seo' && (
            <div className="space-y-10 animate-in fade-in slide-in-from-bottom-4">
              <div className="glass p-10 rounded-[3rem] border border-white/10 space-y-8">
                <h3 className="text-xl font-bold text-white">Search Indexing</h3>
                <div className="space-y-6">
                  <div className="space-y-3">
                    <label className="text-[10px] font-black text-gray-500 uppercase tracking-widest">Metadata Title</label>
                    <input value={formData.seo?.title} onChange={e => setFormData({ ...formData, seo: { ...formData.seo!, title: e.target.value } })} className="w-full bg-white/5 border border-white/10 rounded-2xl py-4 px-8 text-white outline-none" />
                  </div>
                  <div className="space-y-3">
                    <label className="text-[10px] font-black text-gray-500 uppercase tracking-widest">Keywords (comma separated)</label>
                    <input value={formData.seo?.keywords} onChange={e => setFormData({ ...formData, seo: { ...formData.seo!, keywords: e.target.value } })} className="w-full bg-white/5 border border-white/10 rounded-2xl py-4 px-8 text-white outline-none" />
                  </div>
                  <div className="space-y-3">
                    <label className="text-[10px] font-black text-gray-500 uppercase tracking-widest">Public Description</label>
                    <textarea rows={4} value={formData.seo?.description} onChange={e => setFormData({ ...formData, seo: { ...formData.seo!, description: e.target.value } })} className="w-full bg-white/5 border border-white/10 rounded-2xl py-4 px-8 text-white outline-none" />
                  </div>
                </div>
              </div>
            </div>
          ...
          <div className="flex flex-col sm:flex-row gap-6 pt-12">
             ...
             <button type="button" onClick={() => navigate('/admin')} className="px-12 py-6 glass text-gray-500 font-black rounded-[2rem] hover:text-red-500 transition-all uppercase tracking-widest text-sm">Abort Operation</button>
          </div>
        </form>
      </div>
        ...
        <div className="fixed inset-0 z-[100] bg-black/95 flex items-center justify-center p-4 animate-in fade-in">
          <div className="glass w-full max-w-5xl max-h-[90vh] rounded-[3rem] border border-white/10 flex flex-col overflow-hidden">
            ...
            <div className="flex-grow overflow-y-auto p-8 grid grid-cols-2 md:grid-cols-4 xl:grid-cols-5 gap-6">
              ...
              {media.length === 0 && (
                <div className="col-span-full py-20 text-center text-gray-600 font-bold uppercase tracking-widest">Repository Empty</div>
              )}
            </div>
            
            <footer className="p-8 border-t border-white/10 text-center">
